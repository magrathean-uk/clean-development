...
| Claude Code | Install through the Claude marketplace, which selects `claude-skills/`. That variant sets `disable-model-invocation: true`. | `clean-development agent claude --session session-only -- ...` |
...
- **Claude:** frontmatter and marketplace selection were checked against the [Claude skill contract](https://code.claude.com/docs/en/skills). No local Claude executable was available for a live loader check in this environment.
